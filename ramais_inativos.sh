...
function lastX {
	...
	mapfile -t extensionCallOut < <(mysql ${sqlOptions} --batch -N futurofone -e 'SELECT DISTINCT 'src_cdr' AS origem FROM 'pbx_cdr' WHERE 'calldate_cdr' BETWEEN DATE_ADD( CURRENT_DATE( ) , INTERVAL '${parameter}' DAY ) AND DATE_ADD( CURRENT_DATE( ) , INTERVAL +1 DAY ) AND CHAR_LENGTH( 'src_cdr' ) <5 AND 'src_cdr'>99 AND 'direcao_cdr'="saida" AND 'aplicativo_cdr'="Ramal" ORDER BY 'src_cdr' ASC')

        mapfile -t extensionCallIn < <(mysql ${sqlOptions} --batch -N futurofone -e 'SELECT DISTINCT 'dst_cdr' AS destino FROM 'pbx_cdr' WHERE 'calldate_cdr' BETWEEN DATE_ADD( CURRENT_DATE( ) , INTERVAL '${parameter}' DAY ) AND DATE_ADD( CURRENT_DATE( ) , INTERVAL +1 DAY ) AND CHAR_LENGTH( 'dst_cdr' ) <5 AND 'dst_cdr'>99 AND 'direcao_cdr'="saida" AND 'aplicativo_cdr'="Ramal" AND 'dialstatus_cdr'="answer" ORDER BY 'dst_cdr' ASC')
