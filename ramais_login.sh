...
function lastX {
        ...
        mapfile -t extensionAll < <(mysql ${sqlOptions} --batch -N futurofone -e 'SELECT 'numero_ramal' FROM 'pbx_ramal' ORDER BY 'numero_ramal' ASC')
        mapfile -t extensionAllPa < <(mysql ${sqlOptions} --batch -N futurofone -e 'SELECT 'numero_ramal' FROM 'pbx_ramal' WHERE 'tipo_ramal'="2" OR 'tipo_ramal'="5" ORDER BY 'numero_ramal' ASC')
        mapfile -t extensionLogin < <(mysql ${sqlOptions} --batch -N futurofone -e 'SELECT DISTINCT 'ramal_cdrqueuelogin' AS ramal FROM 'pbx_cdrqueuelogin' WHERE 'time_cdrqueuelogin' BETWEEN DATE_ADD( CURRENT_DATE( ) , INTERVAL '${parameter}' DAY ) AND DATE_ADD( CURRENT_DATE( ) , INTERVAL +1 DAY ) ORDER BY 'ramal_cdrqueuelogin' ASC' | grep -v NULL)
